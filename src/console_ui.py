...
class ConsoleUI:
    ...
    def format_weather_list(data):
        ...
        dividing_line = "-" * len(header)
        lines = [header, dividing_line]
    ...
    def format_vehicle_list(data):
        ...
        for number, item in enumerate(data, start=1):
            lines.append(
                f"{number:<4}"
                f"{item.name:<38}"
                f"{item.battery_capacity_kwh:>8.1f} kWh"
    ...
    @staticmethod
    def print_vehicle():
        data_vehicle = DataLoader.load_vehicle_profiles()
        print(ConsoleUI.format_vehicle_list(data_vehicle))
