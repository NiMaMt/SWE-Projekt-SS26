...
# Funtkion zum laden einer JSON-Datei, mit Rückgabe eines spezifischen Attributs aus allen Profilen
def load_json_attributes(choose_file: str, choose_attribute: str):
    ...
    if choose_file == "vehicleprofile":
        ...
        print(f"Fehler: Die Datei '{choose_file}.json' ist unbekannt oder wird nicht unterstützt.")
        return

    # Überprüfung, ob Daten erfolgreich geladen wurden TODO Überprüfung ist eine Ergänzung von KI -> in AI.md übernehmen
    if data is None:
        print(f"Konnte Daten für '{choose_file}.json' nicht laden. Bitte überprüfe die Datei und die Ladefunktion.")
        return
    ...
    if profile_key not in data or not isinstance(data[profile_key], list):
        print(f"Fehler: Der Schlüssel '{profile_key}' wurde in '{choose_file}.json' nicht gefunden oder ist keine Liste von Profilen.")
        return
